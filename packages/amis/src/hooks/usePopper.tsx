...
// https://github.com/kingdee/kdesign/blob/main/components/popconfirm/style/index.less
...
function getTranslate(node: Element, param: string) {
  const translates =
    document.defaultView?.getComputedStyle(node, null).transform.substring(7) ||
    '';
...
const getTranslatePos: (el: Element) => {top: number; left: number} = (
  el: HTMLElement
) => {
  const elPos = {
...
const getOffsetPos: (el: Element) => {top: number; left: number} = (
  el: HTMLElement
) => {
...
const getBorderWidth: (el: Element) => {top: number; left: number} = (
  el: HTMLElement
) => {
  ...
  if (el.offsetParent) {
    ...
    border.top +=
      parentBorder.top +
      parseInt(
        getComputedStyle(el.offsetParent, null).borderTopWidth.slice(0, -2)
      );
    border.left +=
      parentBorder.left +
      parseInt(
...
const getScrollDist: (
  ...
  const isFixed =
    getComputedStyle(el, null).getPropertyValue('position') === 'fixed';
  if (isFixed) {
...
const getRealDom = (locatorRef: any, locatorElement: any) => {
  ...
  if (ReactDOM?.findDOMNode) {
    const res = ReactDOM?.findDOMNode(locatorRef.current);
    if (res) {
      return res;
    }
  }
  const name = REF_NAME_OBJ[locatorElement?.type?.displayName];
  return locatorRef?.current[name] ?? locatorRef.current;
};

function usePopper(
  locatorElement: React.ReactElement,
  popperElement: React.ReactElement,
  props: PopperProps
) {
  const {
    prefixCls,
  ...
  devWarning(
    isWrongTrigger,
    componentName,
    `cannot found ${componentName} trigger '${trigger}'`
  );
  ...
  const container = getPopupContainer(
    getRealDom(locatorRef, locatorElement) || document.body
  );
  ...
  const alignPopper = useCallback(() => {
    ...
    if (realDom && popperRef?.current) {
      ...
      const {top: scrollTop, left: scrollLeft} = getScrollDist(
        realDom.parentElement,
        container
      );
      ...
      const locatorPos = {
        ...
        left:
          locatorLeft + borderLeft + translateLeft - containerLeft - scrollLeft,
        right:
          locatorLeft +
          borderLeft +
          translateLeft +
      ...
      if (autoPlacement) {
        if (
          top - gap - popperHeight <= 5 &&
          bottom + gap + popperHeight < document.body.clientHeight - 5
        ) {
        ...
        if (
          left + popperWidth >= document.body.clientWidth - 5 &&
          right - popperWidth > 5
        ) {
        ...
        if (
          top + popperHeight >= document.body.clientHeight - 5 &&
          bottom - popperHeight > 5
        ) {
          currentPlacement = currentPlacement.replace('Top', 'Bottom');
        }
        if (
        ...
        if (
          left - gap - popperWidth <= 5 &&
          right + gap + popperWidth < document.body.clientWidth - 5
        ) {
          currentPlacement = currentPlacement.replace('left', 'right');
        }
        if (
      ...
      if (/left/.test(currentPlacement) || /right/.test(currentPlacement)) {
        ...
        if (top <= 0) {
          ...
          const scrollBottom =
            alignPos.top + popperHeight - 5 - window.pageYOffset;
          if (scrollTop < 0) {
        ...
        /top/.test(currentPlacement) ||
        /bottom/.test(currentPlacement)
      ) {
  ...
  const popperProps: NormalProps = {
    ...
    onTransitionEnd:
      typeof onTransitionEnd === 'function' ? onTransitionEnd : undefined
  };
  ...
  const matchTrigger = (words: TriggerType) =>
    Array.isArray(trigger) ? trigger.includes(words) : trigger === words;
    ...
    if (exist && visible) {
      ...
      const handleHidePopper = (e: MouseEvent) => {
        ...
        const inPopperRect =
          ...
          Y > popperRect.top &&
          Y < popperRect.bottom;
        const ableArea = matchTrigger('contextMenu')
          ? inPopperRect
          : inTriggerRect || inPopperRect;
      ...
      Array.isArray(trigger)
        ? trigger.forEach((action: string) =>
            document.addEventListener(mapEvent[action], debounceHidePopper)
          )
      ...
      return () => {
        ...
        Array.isArray(trigger)
          ? trigger.forEach((action: string) =>
              document.removeEventListener(mapEvent[action], debounceHidePopper)
            )
  ...
  useEffect(() => {
    if (visible) {
      const scrollAlign = debounce((e: Event) => {
        const isPopperScroll =
          e.target === popperRef.current ||
      ...
      return () => {
        ...
        exist &&
          popperNode?.removeEventListener('DOMSubtreeModified', alignPopper);
      };
    }
  }, [
    alignPopper,
  ...
  useEffect(() => {
    ...
    if (triggerNode) {
      ...
      const debounceShowPopper = debounce(handleShowPopper, 10, {
        leading: true
      });
